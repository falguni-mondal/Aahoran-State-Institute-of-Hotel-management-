...
export default function FTHero() {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo('.ft-hero-bg', 
      { scale: 1.15 }, 
      { scale: 1, duration: 2.5, ease: 'expo.out' }
    );

    // Staggered text reveal
    const heading = document.querySelector('.ft-heading');
    let splitHeading;
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-[100svh] min-h-[600px] bg-[var(--primary-base)] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 w-full h-full z-0 overflow-hidden">
        <img 
          src="/ft-hero.webp" 
          alt="Full Term Hospitality Degree" 
          className="ft-hero-bg absolute inset-0 w-full h-full object-cover will-change-transform"
        />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-5 md:px-8 lg:px-12 xl:px-16 2xl:px-24 w-full">
        <span className="ft-hero-sub text-[10px] md:text-xs lg:text-sm font-semibold uppercase tracking-[0.3em] text-[var(--accent)] mb-4 md:mb-6 lg:mb-8 block">
          Flagship Degree Programme
        </span>
        <h1 className="ft-heading head-txt text-5xl md:text-7xl lg:text-8xl xl:text-9xl 2xl:text-[10rem] leading-[0.9] tracking-tighter text-[var(--text-light)] uppercase max-w-[90vw]">
          Full Term <br /> Courses.
        </h1>
      </div>
