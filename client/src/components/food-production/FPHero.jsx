...
export default function FPHero() {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo('.fp-hero-bg', 
      { scale: 1.15 }, 
      { scale: 1, duration: 2.5, ease: 'expo.out' }
    );

    // Staggered text reveal
    const heading = document.querySelector('.fp-heading');
    let splitHeading;
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-[100svh] min-h-[600px] bg-[var(--primary-base)] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-5 md:px-8 lg:px-12 xl:px-16 2xl:px-24 w-full">
        <span className="fp-hero-sub text-[10px] md:text-xs lg:text-sm font-semibold uppercase tracking-[0.3em] text-[var(--accent)] mb-4 md:mb-6 lg:mb-8 block">
          Culinary Excellence
        </span>
        <h1 className="fp-heading head-txt text-5xl md:text-7xl lg:text-8xl xl:text-9xl 2xl:text-[10rem] leading-[0.9] tracking-tighter text-[var(--text-light)] uppercase max-w-[90vw]">
          Food <br /> Production.
        </h1>
      </div>
