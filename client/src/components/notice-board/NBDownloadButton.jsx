...
export default function NBDownloadButton() {
  ...
  const handleMouseLeave = (e) => {
    ...
    gsap.to([textRef.current, iconRef.current], { color: "var(--primary-base)", duration: 0.3, overwrite: true });
  };
  ...
  return (
    <button
      ...
      className="relative overflow-hidden flex items-center justify-center gap-2 border border-[var(--primary-base)]/20 px-5 py-3 md:px-6 md:py-3.5 rounded-full outline-none transition-colors duration-300 w-full md:w-auto shrink-0 cursor-pointer"
    >
      {/* 
      ...
      */}
      <span
        ref={flairRef}
        className="pointer-events-none absolute w-[200px] h-[200px] bg-[var(--primary-base)] rounded-full z-0"
      ...
      <svg 
        ref={iconRef}
        className="w-3.5 h-3.5 md:w-4 md:h-4 text-[var(--primary-base)] relative z-10" 
        fill="none" 
        stroke="currentColor" 
        viewBox="0 0 24 24"
      >
      ...
      <span 
        ref={textRef}
        className="font-sans text-[10px] md:text-[11px] font-bold uppercase tracking-[0.15em] text-[var(--primary-base)] relative z-10"
      >
        Download
      </span>
    </button>
